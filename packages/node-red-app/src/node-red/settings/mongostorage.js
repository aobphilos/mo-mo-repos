...
function encodeJson(json) {
    ...
    //console.log(json);
    var ret = JSON.parse(JSON.stringify(json).replace(/\$/g, "_DOLLAR_").replace(/\./g, "_DOT_"));
    //console.log(ret);
    return ret;
}
function decodeJson(json) {
    ...
    //console.log(json);
    var ret = JSON.parse(JSON.stringify(json).replace(/_DOLLAR_/g, "\$").replace(/_DOT_/g, "."));
    //console.log(ret);
    return ret;
}
...
function db() {
    return when.promise(function(resolve,reject,notify) {
        if (!mongodb) {
            var shard = (process.env.ndb.split("/")[0].indexOf(",")>=0);
            console.log("connecting to ",process.env.ndb," with sharding ",shard);
            mongo.connect("mongodb://"+process.env.ndb,
                {
                    sslValidate: false,
                    poolSize:5,
                    auto_reconnect:true,
...
var mongostorage = {
    init: function(_settings) {
        settings = _settings;
        appname = process.env.nuser+"__messagelogic";
        return db();
    },
    getFlows: function() {
