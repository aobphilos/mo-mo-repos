...
module.exports = function (RED) {
    ...
    function EnrichResponse(n) {
        RED.nodes.createNode(this,n);
        var node = this;
        this.on("input",function(msg) {
            if (msg.res) {
                log.debug("[Enrich-resp] Responding with ",msg.payload );
                var statusCode = msg.statusCode || 200;
                if (typeof msg.payload == "object" && !Buffer.isBuffer(msg.payload)) {
                    msg.res._res.status(statusCode).json(msg.payload);
                } else {
    ...
    RED.nodes.registerType("enrich-resp",EnrichResponse);
    //---------------------------------------- END OF NODE ---------------------------------------
}
